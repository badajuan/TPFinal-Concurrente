    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path):
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                content = file.read()
                content = re.sub(r'T0', '', content)    #Filtro las transiciónes T0 del log ya que no afectan los invariantes
                replacements_count = 0
                #for i in range(0, 4):
                while True:
                    result, replacements = re.subn(invariants, keep, content) #Filtra todos los invariantes que puede y deja los restos
                    replacements_count += replacements                  #Guarda el conteo de reemplazos
                    if (result == content)|(len(result)==0) : break     #Si no hizo ningun reemplazo, detiene el loop
                    else: content = result                              #Guarda el nuevo resultado para el proximo filtro
                    print(len(result))                                  #Imprime el largo parcial del resto
